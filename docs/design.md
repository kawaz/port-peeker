...
エージェント自体の死活確認。常に 200、ボディは `OK\n`。port-peeker プロセスがリクエストを処理できる = ホスト全体が生きている、と読み替えられる。

**想定用途** (詳細は [decisions/DR-008-healthz-as-host-liveness-fallback.md](decisions/DR-008-healthz-as-host-liveness-fallback.md)):

- ACME `HTTP-01` 認証用 `:80` リスナーのような**断続稼働ポート**のターゲットグループ向け。`/check?port=80` だと常時 503 になってしまうケースの逃げ場
- sshd 等の**セキュリティ重要ポート**を NLB の TCP プローブ対象から外すための代替ヘルスチェック。ヘルスチェック起因の接続ログでインシデント検知が埋もれるのを避ける用途
...
> デフォルトポート `:24365` は「**24** 時間 **365** 日ホストを覗き続けるエージェント」という意味付け。常駐デーモンのキャラ性 ([decisions/DR-007-naming-port-peeker.md](decisions/DR-007-naming-port-peeker.md)) と揃えた小ネタ。技術的根拠はないので運用上の理由で別ポートに変えても問題ない。
